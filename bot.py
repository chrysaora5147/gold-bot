...
POSITION_MODE = os.getenv("POSITION_MODE", "overlay_80_40").strip().lower()
...
POSITION_MODES = {
    "dynamic": None,
    "overlay_80_40": {"down_exp": 0.40, "up_exp": 0.80},
    "overlay_90_50": {"down_exp": 0.50, "up_exp": 0.90},
}
...
def normalize_position_mode(mode):
    if mode in POSITION_MODES:
        return mode
    print(f"⚠️ Unknown POSITION_MODE={mode!r}; falling back to overlay_80_40")
    return "overlay_80_40"


RISK_PROFILE = normalize_risk_profile(RISK_PROFILE)
POSITION_MODE = normalize_position_mode(POSITION_MODE)
...
def calculate_overlay_position(direction, mode="overlay_80_40"):
    overlay = POSITION_MODES[normalize_position_mode(mode)]
    if overlay is None:
        return None
    return float(overlay["up_exp"] if direction == "UP" else overlay["down_exp"])
...
def run_walk_forward_backtest(history, feature_cols, lookback=252 * 3, test_days=252):
    ...
    for idx in range(start, len(history)):
        ...
        volatility = float(test['gold_volatility'].iloc[0])
        threshold = threshold_for_regime(ema_value, RISK_PROFILE)
        ...
        dynamic_position = calculate_target_position(proba, threshold, ema_value, volatility, RISK_PROFILE)
        overlay_position = calculate_overlay_position(pred, POSITION_MODE)
        audit_position = dynamic_position if overlay_position is None else overlay_position
        ...
        rows.append({
            ...
            "strategy_return": audit_position * fwd_return,
        })
    ...
    return {
        ...
        "avg_position": float(audit['position'].mean()) if total else 0,
...
# ตรรกะคุมพอร์ต Allocation: overlay 80/40 คือถือทอง 80% เมื่อ UP และ 40% เมื่อ DOWN.
dynamic_position = calculate_target_position(raw_proba, current_thresh, ema_crossover, gold_volatility, RISK_PROFILE)
overlay_position = calculate_overlay_position(model_direction, POSITION_MODE)
target_position = dynamic_position if overlay_position is None else overlay_position
print(
    ...
    f"risk_profile={RISK_PROFILE} | position_mode={POSITION_MODE} | "
...
if backtest_summary:
    audit_line = (
        ...
        f"avg position: {backtest_summary['avg_position']:.1%} | "
